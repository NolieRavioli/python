...
YT_PLAYLIST = 'PLAYLIST_ID_HERE'

# Setup headers [WILL BE DIFFERENT!!]
setup(
    filepath=os.path.join("downloads","headers_auth.json"),
    headers_raw="""PLACE_HEADERS_HERE"""
)
print("[INFO] Headers saved to downloads/headers_auth.json!")
script_dir = os.path.dirname(os.path.abspath(__file__))
os.environ["PATH"] = os.path.join(script_dir, "ffmpeg\bin;") + os.environ["PATH"]
# Paths
archive_path = os.path.join(downloads_folder,"archive.txt")

# Use %APPDATA% to build the path to yt-dlp.exe [MAY BE DIFFERENT!]
yt_dlp_exe = os.path.join(script_dir, "Scripts", "yt-dlp.exe")

sort_folder = os.path.join(script_dir, "sort")
...
def main():
    ...
    if YT_PLAYLIST == 'LM':
        playlist = ytmusic.get_liked_songs(limit=2000)
    else:
        playlist = ytmusic.get_playlist(YT_PLAYLIST, limit=2000)
